...
def P2sRt(P):
    ...
    # Zhiqi
    # Refine the rotation matrix to the closest orthogonal matrix
    U, S, Vt = linalg.svd(R)
    R = U @ Vt
...
#### -------------------------------------------3. estimate transform matrix using 3D landamrks.

def estimate_transformation_3D(V, M, R):   
    #--- 1. Estimate scale 
    '''
    Args:
        V: (3, n) 
    ...
    M_mean = M.mean(axis = 1).reshape((-1, 1))
    
    V_centered = V - V_mean
    M_centered = M - M_mean
    
    s_3d = np.sqrt((M_centered**2).sum()/((R@V_centered)**2).sum())
        
    # Cloced form solution V.T R.T = M.T
    sV_centered = s_3d * V_centered
    R_t = np.linalg.inv(sV_centered@sV_centered.T) @ sV_centered @ M_centered.T  
    R_3d = R_t.T

    t_3d = M_mean - s_3d * R_3d @ V_mean
    t_3d = t_3d.flatten()
    
    return s_3d, R_3d, t_3d
